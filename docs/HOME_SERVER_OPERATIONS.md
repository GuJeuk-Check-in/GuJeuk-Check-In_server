...
| 구분 | 주소 | 내부 연결 |
|---|---|---|
| 운영 API | `https://api.taisu.site` | oijwef `taisu-oijwef` Tunnel -> `http://localhost:8080` |
| Oracle HAProxy | `http://161.33.21.41` | `api.taisu.site` / `api.oijwef098234.com` 로드밸런싱 |
| Oracle HAProxy Tunnel | `https://proxy.oijwef098234.com` | Oracle cloudflared -> HAProxy `localhost:80` |
| 스테이징 API 라우트 | `https://api-stag.taisu.site` | oijwef `taisu-oijwef` Tunnel -> `http://localhost:8081` |
...
| 원격 SSH | `ssh.taisu.site` | oijwef `taisu-oijwef` Tunnel -> `ssh://localhost:22` |
...
### Oracle HAProxy 로드밸런서

2026-07-07 KST에 Oracle Always Free VM `oracle-haproxy`를 HAProxy 로드밸런서로 구성했다.
...
HAProxy HTTPS frontend: 0.0.0.0:443 with self-signed certificate
Cloudflare Tunnel: oracle-haproxy / 6582395b-e23e-4d68-800d-198dd71bdb48
Proxy route: https://proxy.oijwef098234.com -> Oracle cloudflared -> http://localhost:80
Stats: http://127.0.0.1:8404/haproxy?stats (VM 내부 전용)
```
...
HAProxy는 backend별 Cloudflare Host/SNI가 달라야 하므로 각 backend에서 `Host`와 SNI를 해당 Cloudflare hostname으로 강제 설정한다. 2026-07-14 KST 최종 이전 후에는 `gujeuk_gaemi_api`가 primary이며, `gujeuk_ubuntu_api`는 fallback 조건으로만 남아 있다. `api.taisu.site` 자체도 oijwef의 `taisu-oijwef` Cloudflare Tunnel을 통해 oijwef API로 직접 전달된다.
...
taisu API route: https://api.taisu.site -> oijwef cloudflared-taisu-oijwef -> local gujeuk-app -> 200
...
- VM 내부 iptables는 TCP 80/443 허용 rule과 `gujeuk-allow-http.service`로 재부팅 후 복구를 구성했다.
- 2026-07-07 확인 시 OCI inbound TCP 80을 허용한 뒤 public HTTP 로드밸런싱은 정상이다.
- 2026-07-07 확인 시 `161.33.21.41:443`은 외부에서 timeout이다. Cloudflare SSL mode `Full`을 쓸 경우 OCI VCN Security List 또는 NSG에서 inbound TCP 443도 허용해야 한다. `Flexible`로 빠르게 전환하면 80만으로 동작한다.
- 2026-07-07에 `proxy.oijwef098234.com`은 Oracle 전용 Cloudflare Tunnel로 연결했다. 이 경로는 Cloudflare edge -> Oracle `cloudflared` -> local HAProxy `:80` 구조라 OCI public 443 개방이 없어도 HTTPS로 동작한다.
- `api.taisu.site`를 Oracle HAProxy로 전환하기 전 `http://161.33.21.41/purpose/all` 또는 임시 DNS로 public 200을 먼저 확인한다.
...
2026-07-14 KST 기준 oijwef 장비(`ant`, 사용자 `gaemideul8`)도 같은 덮개 닫힘 방지 설정이 적용되어 있다.
...
`systemd-analyze cat-config systemd/logind.conf` 기준 effective config에도 위 세 값이 반영되어 있다. oijwef는 `Linger=yes`이며 `cloudflared.service`와 `cloudflared-taisu-oijwef.service`가 사용자 systemd 서비스로 enabled/active 상태다.
...
api.taisu.site                  -> oijwef cloudflared-taisu-oijwef -> oijwef API, 200
...
local app tunnel: 172.18.0.1:13306 -> gaemideul8 127.0.0.1:3306 through Cloudflare SSH
local replica tunnel: 172.28.0.1:13306 -> gaemideul8 127.0.0.1:3306 through Cloudflare SSH
...
2026-07-14 KST 이후 `taisu.site` 공개 호스트는 oijwef의 전용 Tunnel `taisu-oijwef`로 이전했다. oijwef의 `/home/gaemideul8/.cloudflared/taisu-oijwef.yml`에는 다음 구조를 적용했다.
...
systemctl --user status cloudflared-taisu-oijwef.service
...
routes:
  ...
  worldcup.taisu.site  -> http://localhost:8789
  monitor.taisu.site   -> http://localhost:3000
  poker.taisu.site     -> http://localhost:8791
...
현재 정상 구조는 oijwef의 `gujeuk-mysql`이 운영 primary DB이고, oijwef `gujeuk-app`이 이를 바라보는 형태다. `api.taisu.site`/`api-stag.taisu.site` 경로도 oijwef의 `cloudflared-taisu-oijwef.service`를 통해 oijwef API로 직접 전달된다.
